...
class IntervalTimer {
  ...
  Duration totalDurationImpl() => timers.fold<Duration>(
    Duration.zero,
    (p, v) => p + v
  );
...
void partialApplicationTests() {
  ...
  dynamic combinedPartial = sayHi.partial(['Charlie', 'Gameti'], {'excited': true});
...
void memoizationTests() {
  ...
  dynamic expensiveComputation = VarArgsFunction<int>((args, _) {
    ...
    if (args.length == 1 && args[0] is int) {
      ...
      throw ArgumentError('Expected a single integer or a list containing an integer');
    }
    ...
    print('Computing fibonacci($n) - call #$computeCount (took ${elapsed / 1000} ms)');
  ...
  print('\n📊 Running benchmarks...');
  print('┌${'─' * 60}┐');
  print('│ ${'Test'.padRight(15)}│ ${'Value'.padRight(8)}│ ${'Time (ms)'.padRight(15)}│ ${'Result'.padRight(15)}│');
  print('├${'─' * 60}┤');
  ...
  for (final n in testValues) {
    ...
    print('│ ${'Non-memoized'.padRight(15)}│ ${n.toString().padRight(8)}│ ${(elapsed / 1000).toStringAsFixed(3).padRight(15)}│ ${result.toString().padRight(15)}│');
  }
  ...
  for (final n in testValues) {
    ...
    print('│ ${'Memoized (1st)'.padRight(15)}│ ${n.toString().padRight(8)}│ ${(elapsed / 1000).toStringAsFixed(3).padRight(15)}│ ${result.toString().padRight(15)}│');
  }
  ...
  for (final n in testValues) {
    ...
    print('│ ${'Memoized (2nd)'.padRight(15)}│ ${n.toString().padRight(8)}│ ${(elapsed / 1000).toStringAsFixed(3).padRight(15)}│ ${result.toString().padRight(15)}│');
  }
  ...
  print('\n📈 Performance Summary:');
  print('┌${'─' * 60}┐');
  print('│ ${'Value'.padRight(8)}│ ${'Speedup (1st call)'.padRight(20)}│ ${'Speedup (2nd call)'.padRight(20)}│');
  print('├${'─' * 60}┤');
  ...
  for (final n in testValues) {
    ...
    print('│ ${n.toString().padRight(8)}│ ${firstCallSpeedup.toStringAsFixed(2).padRight(20)}x│ ${secondCallSpeedup.toStringAsFixed(2).padRight(20)}x│');
  }
  
  print('└${'─' * 60}┘');
  
  // Calculate average speedup
  final avgFirstCallSpeedup = testValues.map((n) => 
      results['non-memoized']![n]! / results['memoized-first']![n]!).reduce((a, b) => a + b) / testValues.length;
      
  final avgSecondCallSpeedup = testValues.map((n) => 
      results['non-memoized']![n]! / results['memoized-second']![n]!).reduce((a, b) => a + b) / testValues.length;
  
  print('\n🚀 On average, memoization provides:');
  print('   - ${avgFirstCallSpeedup.toStringAsFixed(2)}x speedup on first call');
  print('   - ${avgSecondCallSpeedup.toStringAsFixed(2)}x speedup on subsequent calls');
...
void debounceTests() {
  ...
  print('only execute once after the specified delay, even if called multiple times.');
...
void throttleTests() {
  ...
  print('- Subsequent calls within the throttle period (2 seconds) are ignored');
