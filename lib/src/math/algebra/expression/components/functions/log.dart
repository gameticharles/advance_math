...
class Log extends Expression {
  ...
  dynamic evaluate([dynamic arg]) {
    ...
    if (operandValue is Matrix) {
      if (baseValue is! num) {
        throw ArgumentError(
            'Logarithm of a matrix with a non-scalar base is not supported.');
      }
    ...
    if (baseValue is Expression || operandValue is Expression) {
      return Log(
        baseValue is Expression ? baseValue : Literal(baseValue),
        operandValue is Expression ? operandValue : Literal(operandValue),
      ).simplify();
    }
