...
abstract class Real extends Number {
  ...
  Number operator ~/(dynamic divisor) {
    ...
    if (divisor is Precise) return (Precise.num(value) / divisor).truncate();
  ...
  Number operator ^(dynamic exponent) {
    ...
    if (exponent is Precise) return (Precise.num(value)) ^ exponent;
    if (exponent is Real) {
  ...
  bool operator >(dynamic obj) {
    ...
    if (obj is Precise) return Precise.num(value) > obj;
