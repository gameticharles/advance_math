  ...
  group('Parser Calculus & Solver Comprehensive Tests', () {
    ...
    group('Differentiation Edge Cases (30 tests)', () {
      ...
      test(
          ...
          () => expect(
              ...
              anyOf(contains('-2*x^(-3)'), contains('x^-2*-2/x'),
                  contains('x^(-2)*-2/x'))));
    ...
    group('Integration Edge Cases (40 tests)', () {
      ...
      test(
          'integrate(cos(x/2), x)',
          () => expect(
              parser.parse('integrate(cos(x/2), x)').toString(),
              anyOf(contains('2*sin((1/2)*x)'), contains('sin((x / 2))'),
                  contains('sin(x/2)'))));
      test(
          'integrate((2*x+1)^2, x)',
    ...
    group('Solver Edge Cases (30 tests)', () {
      ...
      test(
          'solve(x^2 - 2, x)',
          () => expect(parser.parse('solve(x^2 - 2, x)').toString(),
              anyOf(contains('1.414'), contains('sqrt(2)'))));
      test(
          'solve(2*x^2 - 8, x)',
          () => expect(
      ...
      test(
          'solve(0*x - 5, x)',
          () =>
              expect(() => parser.parse('solve(0*x - 5, x)'), isA<dynamic>()));
      test('solve(1, x)',
